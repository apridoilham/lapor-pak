...
@section('content')
    ...
    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" id="profile-form">
        ...
        <div class="form-section-card">
            ...
            <div class="mb-3"><label for="name" class="form-label">Nama Lengkap</label><input type="text" class="form-control" id="name" value="{{ $user->name }}" readonly disabled></div>
            <div class="mb-3"><label for="email" class="form-label">Email</label><input type="email" class="form-control" id="email" value="{{ $user->email }}" readonly disabled></div>
            <div class="mb-3"><label for="phone" class="form-label">Nomor Telepon</label><input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $user->resident->phone) }}" placeholder="Contoh: 081234567890">@error('phone')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
        </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const checkForChanges = () => { /* (Logika tidak berubah) */ };
            form.addEventListener('input', checkForChanges);
            form.addEventListener('change', checkForChanges);

            const avatarInput = document.getElementById('avatar-input');
            const avatarPreview = document.getElementById('avatar-preview');
            avatarInput.addEventListener('change', (event) => { /* (Logika tidak berubah) */ });
            
            // ▼▼▼ SKRIP UNTUK LOKASI DAN DROPDOWN ▼▼▼
            ...
            const defaultLat = {{ $user->resident && $user->resident->latitude ? $user->resident->latitude : -6.3816 }};
            const defaultLng = {{ $user->resident && $user->resident->longitude ? $user->resident->longitude : 106.7420 }};
            ...
            const fetchRts = (rwId, selectedRtId = null) => {
                if (!rwId) { rtSelect.innerHTML = '<option value="">Pilih RW</option>'; rtSelect.disabled = true; return; }
